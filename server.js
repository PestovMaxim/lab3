...
app.get('/books', async (req, res) => {
    ...
    let secret = user + "secret12345";
    let hash_secret = crypto.createHash('md5').update(secret).digest('hex');
    ...
    if(bookname){
        sql = {
            text: "SELECT ba.id, a.name as author, b.name as book FROM books_by_authors ba LEFT JOIN author a ON " +
                "a.id = ba.aid LEFT JOIN book b ON b.id = ba.bid WHERE b.name LIKE '%' || $1 || '%'",
            values: [bookname]
        };
    }
    try{
...
app.post('/signin', async (req, res) => {
    ...
    try{
        ...
        let secret = userId + "secret12345";
        let hash_secret = crypto.createHash('md5').update(secret).digest('hex');
        if(data.rows.length>0 && userId){
            const oneDayToSeconds = 24 * 60 * 60;
            res.cookie('userId', userId,{maxAge: oneDayToSeconds, httpOnly: true}); //httpOnly
            res.cookie('SessionId', hash_secret,{maxAge: oneDayToSeconds, httpOnly: true}); //httpOnly
            res.redirect('/books');
        }else
        {
            res.send(`fail ${login}`);
